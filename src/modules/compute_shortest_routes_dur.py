...
def compute_shortest_routes_dur(
    session: Any,
    run_configs_id: int,
    iteration_id: int
    ...
    - route_objs: List of ORM route objects stored in shortest_routes_duration table
    """
    ...
    try:
        ...
        route_sql = sa_text(f"""
            ...
            filtered_vehicle_routes AS (
                SELECT vehicle_id, route_id, duration
                FROM vehicle_routes
            ...
            shortest_selected_routes AS (
                ...
                ON fvr.vehicle_id = sr.vehicle_id AND fvr.duration = sr.min_value
        ...
        logger.info(f"Selected shortest-duration routes for {len(route_pairs)} vehicles.")

        # Step 2: Delete old records and insert new ones
        session.query(ShortestRouteDur).filter(
            ShortestRouteDur.run_configs_id == run_configs_id,
            ShortestRouteDur.iteration_id == iteration_id
        ).delete()
        ...
        congestion_sql = sa_text(f"""
            ...
            JOIN shortest_routes_duration sr1 
            ...
            JOIN shortest_routes_duration sr2 
